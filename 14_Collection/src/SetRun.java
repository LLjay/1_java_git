...
public class SetRun {
	public static void main(String[] args) {
		...
		// String에 equals() 오버라이딩 => "실제 담긴 문자열"을 가지고 동등비교를 진행하여 일치하면 true/일치하지 않으면 false
		// String에 hashCode() 오버라이딩 => "실제 담긴 문자열"을 가지고 10진수의 형태로 만들어서 반환
		
		
		
		HashSet hs2 = new HashSet();
		hs2.add(new Student("전은지", 30, 100));
		hs2.add(new Student("윤현지", 27, 70));
		hs2.add(new Student("서지안", 27, 100));
		hs2.add(new Student("전은지", 30, 100));
		// HashSet이라는 공간에 객체가 추가될 때마다 동일객체인지 비교
		// 동일객체 : 각 객체마다 hashCode 결과가 일치하고, equals 메소드로 비교 시 true가 나올 때
		...
		System.out.println(hs2.add(new Student("전은지", 30, 100)));
		...
		// hs2.get(1); // 인덱스의 개념도 없고 get메소드 자체가 정의되어있지 않음!! -> 한개씩 무작위로 가져올 수 있음
				...
				// ArrayList담는 방법1. : ArrayList생성 후 addAll메소드를 이용해서 통째로 추가
				ArrayList list = new ArrayList();
				list.addAll(hs2);
				
				// ArrayList담는 방법2. : ArrayList 생성과 동시에 통째로 추가
