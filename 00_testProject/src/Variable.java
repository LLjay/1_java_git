...
//		
		...
		long lin = 230342839402983094l;
		//long lin = 230342839402983094; -> 기본적으로 리터럴에 들어온 수가 너무 크기 때문에 인식 오류가 남...?

		System.out.println("정수 자료형");
		System.out.println(by + "," + sh + "," + in + "," + lo + "," + lin);
		 ...
		 * 문자열
		 * String(객체)
		 * 문자열 같은 경우 할당해야 할 메모리 크기가 너무 가변적이므로
		 * 원시 타입이 아닌 객체로 변수를 만들어 준다.
		 */
		...
		/*
		 * 이스케이프 시퀀스
		 * 문자열 내에서 백스페이스, 탭, 백슬래시, 작은따옴표 등 코드로 사용되는 특수문자를
		 * 표현할 때 사용하는 방식
		 * 
		 * 백스페이스 \b
		 * 탭문자 \t
		 ...
		 * -> 나중에 길어졌을 때 이 수는 상수라고 알아보기 쉽게 하기 위해서
		 * 이름이 둘 이상의 단어로 이루어질 경우 단어를 언더바로 연결해준다. ex) myAge -> MY_AGE
		 */
		
		final int AGE = 90;
		//AGE = 10; 한 번 값을 초기화 하면 더이상 변경할 수 없음
