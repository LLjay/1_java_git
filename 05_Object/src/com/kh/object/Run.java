 ...
 * com.kh.팀명(.또는 프로젝트명)
 * 
 * 1. 객체 지향 언어 : "객체"를 "지향"하는 언어로 "객체중심"으로 돌아간다고 생각하면 된다.
 * 
 * 2. 객체 : 현실세계에 독립적으로 존재(목적이 있고 가치가 있고 의미가 있는 것) 하는 모든 것들을 의미(유형, 무형, 개념, ...)
 * 			현실세계에서는 객체들 간의 상호작용으로 돌아감
 * 
 * 3. 객체 지향 프로그래밍 : 현실세계의 객체들 간의 상호작용
 * 						 프로그래밍을 통해서 가상세계로 구현하는 과정
 * 
 * 4. 구현하고자 하는 프로그램 상의 필요한 객체들을 만들기 위해서(즉 생성하기 위해서)
 * 	  : 클래스라는 틀을 먼저 만들어야 한다. (클래스 : 각 객체들의 속성(데이터)들을 담아낼 그릇 같은 존재)
 * 
 * 5. 추상화 과정
 ...
 * 
 * 	ex) 학생 관리 프로그램
 * 		1) 학생 관련 객체(홍길동 학생, 김연아 학생, 최지원 학생...)
 * 		2) 공통적인 속성 및 기능들(이름, 나이, 주소, 전화번호, 키, 몸무게, 달리기 속도, 머리색, 발사이즈, 먹는다, 잔다, 화장실 간다, ...)
 ...
 *		   나이 : int age;
 *		   키 : double height;
 * 6. 위에 선정된 것들을 가지고 본격적인 프로그램을 할 것
 * 		1) "변수" 만으로 프로그래밍을 한다면?
 * 
 ...
 * 		... 등등의 수백 명에 해당하는 학생들을 만들어야 한다.
 * 
 * 		문제점 -> 수백 명에 해당하는 학생들을 만들 때 수천 개의 변수가 필요하다.
 * 
 * 		2) "배열"로 프로그래밍을 한다면?
 * 		
 * 		*배열 = 하나의 자료형에 여러 개의 값들을 보관할 수 있음
 * 		
 * 		학생들의 이름을 보관하는 배열 String[] name = {"홍길동", "김말똥", ...}
 * 		학생들의 나이를 보관하는 배열 int[] age = {20, 30, ...}
 * 		...
 * 
 * 		배열은 한 번 생성해두면 다시 개수를 수정할 수 없기 때문에 만 개의 데이터를 다시 옮겨야 함
 * 		문제점 : 기존의 학생 정보를 지워야 할 때 뒤의 정보들을 앞으로 땡기는 과정을 진행해야 하는데 혹시라도 실수하면 학생의 정보가 뒤바뀔 수 있다.
 * 				또한 새로운 학생의 정보를 기록해야 할 때 새로운 크기의 배열을 만들어서 기존의 배열로부터 복사한 후 추가해야 한다.
 * 
 * 		3) => 구조체라는 개념이 나옴 (자바에는 없음)
 * 
 * 		*구조체 - 여러 개의 자료형의 여러 개의 값들을 보관할 수 있다.
 * 		String, int 값도 보관할 수 있고 동시에 double도 함께 자료형에 상관없이 여러 개의 값을 동시에 저장할 수 있다.
 * 		=> 나만의 자료형의 집합을 만드는 것 -> 자바에서는 이러한 구조체 개념을 좀 더 확장해서 클래스라는 개념을 만들었다.
 * 
 * 
...
public class Run {
	public static void main(String[] args) {
		...
		// 자바에서 객체로 만들기 위해서는 new라는 키워드를 통해 메모리 공간을 할당해줘야 한다.
